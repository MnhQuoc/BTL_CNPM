        ...
        const fetchData = async () => {
            try {
                const [requestsRes, tutorsRes] = await Promise.all([
                    ...
                    fetch('http://localhost:3001/users?role=tutor&verified=true'),
                ]);
    ...
    const handleApprove = async (requestId, userId) => {
        try {
            await fetch(`http://localhost:3001/users/${userId}`, {
                ...
                body: JSON.stringify({ role: 'tutor', verified: true }),
            });
            ...
            // Reload tutors list
            const updatedTutors = await fetch('http://localhost:3001/users?role=tutor&verified=true');
