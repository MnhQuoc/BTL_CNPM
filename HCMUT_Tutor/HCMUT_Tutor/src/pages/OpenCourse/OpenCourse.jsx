...
const OpenCourse = () => {
  ...
  useEffect(() => {
    ...
    // Lấy các courses hiện tại của tutor từ tutorCourses
    const fetchExistingCourses = async () => {
      try {
        const response = await axios.get('http://localhost:3001/tutorCourses');
        const tutorCourses = response.data.filter(
          course => course.tutorId === user.id.toString()
        );
        setExistingCourses(tutorCourses);
        
        // Nếu đã có môn học, tự động điền tên môn học
        if (tutorCourses.length > 0) {
          setSubjectName(tutorCourses[0].name);
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = error.response?.data?.error || 'Có lỗi xảy ra khi tạo khóa học';
      alert(`❌ ${errorMessage}`);
    }
  };
  ...
  return (
    <Container className="open-course-container">
      <h2 className="mb-4">Mở khoá học mới</h2>

      {existingCourses.length > 0 && (
        <Alert variant="info" className="mb-3">
          <strong>Lưu ý:</strong> Bạn đã có môn học "{existingCourses[0].name}". 
          Bạn chỉ có thể thêm khung giờ mới cho môn học này.
        </Alert>
      )}
          ...
          {existingCourses.length === 0 ? (
            ...
            <>
              ...
              <Form.Text className="text-muted">
                Hoặc bạn có thể nhập tên môn học mới bên dưới
              </Form.Text>
              <Form.Control
                type="text"
                value={subjectName}
                ...
                placeholder="Hoặc nhập tên môn học mới"
                className="mt-2"
                isInvalid={!!errors.subjectName}
              />
            </>
